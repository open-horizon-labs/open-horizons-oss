...
-- Create main tables for endeavors, daily pages, role assertions, and relationships
...
create index if not exists endeavors_active_idx on public.endeavors(user_id) where archived_at is null;
create index if not exists endeavors_archived_at_idx on public.endeavors(archived_at) where archived_at is not null;
