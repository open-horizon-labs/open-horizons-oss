...
export const GET = withSimpleAuth(async (request: NextRequest, user: AuthenticatedUser) => {
  try {
    ...
    let sql = 'SELECT id, endeavor_id, content, created_at, updated_at FROM guardrails'
    const params: any[] = []
    ...
    return NextResponse.json({ guardrails: rows })
  } catch (error) {
...
export const POST = withSimpleAuth(async (request: NextRequest, user: AuthenticatedUser) => {
  try {
    ...
    const { endeavor_id, content } = body
